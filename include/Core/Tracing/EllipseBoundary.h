...
struct RowBoundary {
  ...
  bool IsInsideOuter(int x) const {
    // Если границы не установлены (обе 0) — строка вне эллипса
    if (leftOuter == 0 && rightOuter == 0) return false;
    return (x >= leftOuter && x <= rightOuter);
  }
  bool IsInsideInner(int x) const {
    if (!HasInnerBoundary()) return false;  // нет внутренней границы — нет дыры
