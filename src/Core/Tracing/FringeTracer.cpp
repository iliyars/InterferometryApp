...
namespace Interferometry
{
  ...
  int CFringeTracer::Step(std::vector<CTracerPoint> &line)
  {
    ...
    DBG(
        std::cout << "  -> wrote (" << cx << "," << cy << ")"
                  << " predicted=(" << predX << "," << predY << ")"
                  << " shift=" << std::sqrt((float)(cx - predX) * (cx - predX) + (float)(cy - predY) * (cy - predY))
                  << std::endl;);
    return 0;
  }
  ...
  float CFringeTracer::AverageIntensity(int x, int y) const
  {
    ...
    static const int neighbors[8][2] = {{0, 1}, {0, -1}, {1, 0}, {1, 1}, {1, -1}, {-1, 0}, {-1, 1}, {-1, -1}};
