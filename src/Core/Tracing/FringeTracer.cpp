...
bool CFringeTracer::TraceLine(int startX, int startY,
  ...
  if (m_params.bidirectional && m_tempLine.size() >= 3) {
    ...
    // Склейка: реверс (перевернуть) + прямой ход (с точки 1, без дубля pt[0])
    std::reverse(m_tempLine.begin(), m_tempLine.end());

    // Удалить хвост реверса, совпадающий с началом прямого (pt[0], pt[1],
    // pt[2]) После reverse последние 3 точки = pt[2], pt[1], pt[0] Прямой ход
...
int CFringeTracer::Step(std::vector<CTracerPoint>& line) {
  ...
  if (num_point >= 3 && curIntensity > 0) {
    ...
    // Если яркость упала ниже 50% от средней по предыдущим точкам — стоп
  ...
  if (m_wideLine > maxAllowedWidth) m_wideLine = maxAllowedWidth;
...
bool CFringeTracer::CenterPerpendicular(int& x, int& y, int dx, int dy) {
  ...
  for (int i = 1; i < halfWidth; i++) {
    ...
    if (!IsInside(testX, testY)) return false;  // как return(-1) в оригинале

    float intensity = AverageIntensity(testX, testY);
    if (intensity < m_average) break;  // вышли за полосу — стоп
  ...
  for (int i = 1; i < halfWidth; i++) {
    ...
    if (!IsInside(testX, testY)) return false;
